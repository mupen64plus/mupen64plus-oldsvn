...
static void rsp_uc02_matrix()
{
        ...
        _u32   a = segoffset2addr(rdp_reg.cmd1) & 0x003fffff;
...
#ifdef LOG_ON
        LOG_TO_FILE("%08X: %08X %08X CMD UC2_MOVEMEM",ADDR, CMD0, CMD1);
#endif
        switch((rdp_reg.cmd0 >> 16) & 0xff)
        {
            case 0x80:
                /* we do '>> 1' here because we just need _s16's */
                a = (segoffset2addr(rdp_reg.cmd1) & 0x003fffff) >> 1;
            ...
            case 0x82:
                a = segoffset2addr(rdp_reg.cmd1) & 0x003fffff;
            ...
            case 0x94:
                ...
                a = segoffset2addr(rdp_reg.cmd1) & 0x007fffff;
...
static void rsp_uc02_vertex()
{
        _u32   a = segoffset2addr(rdp_reg.cmd1) & 0x007fffff;
        _u16  len;
        int     v0, i, n;

        len = (_u16)(rdp_reg.cmd0 & 0xffff);
//      v0 = (rdp_reg.cmd0 >> 16) & 0x3f;
//      n = ((len ) >> 9) & 0x07f; // len + 1

        v0 = ((rdp_reg.cmd0 >> 16) & 0xFF)/5;      // Current vertex
        n = (WORD)((rdp_reg.cmd0&0xFFFF) + 1)/0x210;    // Number to copy
...
static void rsp_uc02_displaylist()
{
        _u32 addr = segoffset2addr(rdp_reg.cmd1) & 0x003fffff;
