...
unsigned int RSP::seg2phys(unsigned int seg)
{
...
void RSP::executeDList()
{
   ...
   unsigned int *start = (unsigned int*)(gfxInfo.RDRAM + task->data_ptr);
   //unsigned int length = task->data_size;
...
void RSP::DL()
{
   ...
   unsigned int addr = seg2phys(*(currentCommand+1)) & 0x7FFFFF;
   unsigned int *returnCommand = currentCommand;
   switch(push)
     {
      case 0: // DL_PUSH
	currentCommand = (unsigned int*)(gfxInfo.RDRAM + addr);
	...
	break;
      case 1: // NO_PUSH
	currentCommand = (unsigned int*)(gfxInfo.RDRAM + addr) - 2;
	break;
      default:
...
void RSP::SETOTHERMODE_L()
{
   ...
   unsigned int data = *(currentCommand+1);
