...
void init_varlist()
{
    ...
    for(i=0; i<Num_Columns; i++)
    	gtk_signal_connect(GTK_OBJECT(cell_renderer[i]), "edited", (GtkSignalFunc)on_cell_edit, (gpointer)i);
...
static void on_cell_edit(GtkCellRendererText *renderer, gchar *path, gchar *new_text, gpointer user_data)
{
	...
	switch((int)user_data) //column number
	{
		case Col_Name:
	...
	if((int)user_data != Col_Value)
		gtk_tree_store_set_value(store, &iter, (int)user_data, &newval);
}
